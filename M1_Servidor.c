	...
	int NuevaPartida(char listajugadores[20], int socket)
	{
		...
		else
		{
			...
			while(j<miLista.num && encontrado==0)
			{
				...
				{
					encontrado=1; //Encontramos el JugadorConectado que coincida con el socket.
				}
				else j++;
			}
			//int id=g+1;
			...
			printf("socket anfitrion: %d\n", miTabla[s].jugadores[0].socket); //Asignamos socket del anfitrion.
			char *p = strtok(listajugadores, "/");
			int cont=1;//cont=0 es el anfitrion. Asi que empezamos en 1.
			while(p!=NULL)
			{
				strcpy(miTabla[s].jugadores[cont].nombre,p); //pasamos la listajugadores.
				encontrado=0;
				j=0;
				while(j<miLista.num && encontrado==0) //Recorremos lista jugadores conectados.
				{
					if (strcmp(miLista.Lista[j].nombre,p)==0) 
					{
						encontrado=1; //Cuando encontramos el nombre que coincide.
					}
					else j++;
				}
	...
	void *AtenderCliente (void *socket)
	{
			...
			while (terminar == 0){
				...
				{ //Codigo para Chat.
					...
					int id = atoi(p);
					int g = 0;
					sprintf (respuesta, "16/%s/%s/%d", mensaje, usuario, id); //Enviamos 16/mensaje/usuario/id.
					while(strcmp(miTabla[id].jugadores[g].nombre,"")!=0) //Enviamos a todos los que estan en partida.
					{
				...
				notificador=0;
				if (codigo != 0 && codigo != 13 && codigo != 14 && codigo != 15 && codigo != 16)// if (codigo !=0 && codigo != 8 && codigo != 9)
				{
					printf ("Respuesta: %s\n", respuesta);
					// Enviamos respuesta
					write (sock_conn,respuesta, strlen(respuesta));
